...
gBeta_3_3      <- comp_lapAppr_sameImage(test_Beta(3,3), 0, 1)
...
gBeta_40_20    <- comp_lapAppr_sameImage(test_Beta(40,20), 0, 1)
gBeta_400_200  <- comp_lapAppr_sameImage(test_Beta(400,200), 0, 1)
...
test_BiModNorm <- function(m1, sd1, m2, sd2){
  f <- function(x){
    0.5 * (dnorm(x, mean = m1, sd = sd1) + dnorm(x, mean = m2, sd = sd2))
  }
  return(f)
}

test_MultiModNorm4 <- function(m1, sd1, m2, sd2, m3, sd3, m4, sd4){
  f <- function(x){
    0.25 * (dnorm(x, mean = m1, sd = sd1) + 
              dnorm(x, mean = m2, sd = sd2) +
              dnorm(x, mean = m3, sd = sd3) + 
              dnorm(x, mean = m4, sd = sd4))
  }
  return(f)
}

gBiModNorm_0_1_5_1     <- comp_lapAppr_sameImage(test_BiModNorm(0, 1, 5, 1), -5, 7 )
gBiModNorm_0_2_5_1     <- comp_lapAppr_sameImage(test_BiModNorm(0, 2, 5, 1), -5, 7)
gBiModNorm_n5_1_0_2    <- comp_lapAppr_sameImage(test_BiModNorm(-5, 1, 0, 2),-7, 4)
gBiModNorm_n4_0k5_0_2  <- comp_lapAppr_sameImage(test_BiModNorm(-4, 0.5, 0, 2),-7, 4)

gMultiModNorm4_0_1_4_0k5_5_0k2_7_1k5<- 
  comp_lapAppr_sameImage(test_MultiModNorm4(0, 1, 4, 0.5, 5, 0.2, 7, 1.5), -2, 10 )
gMultiModNorm4_0_1_4_0k5_5_0k2_7_0k4 <- 
  comp_lapAppr_sameImage(test_MultiModNorm4(0, 1, 4, 0.5, 5, 0.2, 7, 0.4), -2, 10 )
gMultiModNorm4_0_1_4_0k5_5_0k2_7_1 <- 
  comp_lapAppr_sameImage(test_MultiModNorm4(0, 1, 4, 0.5, 5, 0.2, 7, 1), -2, 10 )
gMultiModNorm4_0_1_4_0k5_5_0k2_7_2 <- 
  comp_lapAppr_sameImage(test_MultiModNorm4(0, 1, 4, 0.5, 5, 0.2, 7, 2), -2, 10 )
